abstract type AbstractXCCalculator end
...
function XCCalculator(;is_metagga=false, Npoints=0, Nspin=1)
    if is_metagga
        @assert Npoints > 0
        Vlapl = zeros(Npoints, Nspin)
        Vtau = zeros(Npoints, Nspin)
        return XCCalculator(Vlapl, Vtau) # XXX use SCAN
    else
        return XCCalculator(zeros(1, 1), zeros(1, 1))
    end
end
